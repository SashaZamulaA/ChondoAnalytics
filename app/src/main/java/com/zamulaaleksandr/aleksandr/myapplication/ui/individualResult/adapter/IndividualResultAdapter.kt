package com.zamulaaleksandr.aleksandr.myapplication.ui.individualResult.adapter
...
//                result_city.text = model.centers
    ...
    private fun formatTime(ts: Long): String = Calendar.getInstance().run { timeInMillis = ts; timeFormatter.format(time) }
            ...
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NoteHolder {
                val v = LayoutInflater.from(parent.context).inflate(com.zamulaaleksandr.aleksandr.myapplication.R.layout.item_individual_result_list,
                        parent, false)
                return NoteHolder(v)
            }
