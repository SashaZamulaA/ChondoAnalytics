...
class EachCenterAdapter2(private val context: Context,
                         options: FirestoreRecyclerOptions<City>) : FirestoreRecyclerAdapter<City, RecyclerView.ViewHolder>(options) {
...
//    private fun formatTime(ts: Long): String = Calendar.getInstance().run { timeInMillis = ts; timeFormatter.format(time) }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int, model: City) {
    ...
    inner class CenterEachHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind() {
            ...
            itemView.apply {
                ...
                each_center_name.text = item.name
                each_center_time.text = formatDateAndTime(item.timestamp)
                each_center_intro.text = if (!item.intro.isNullOrBlank()) item.intro else "0"
                each_center_one_day_edittext.text = if (!item.onedayWS.isNullOrBlank()) item.onedayWS else "0"
                each_center_two_day.text = if (!item.twoDayWS.isNullOrBlank()) item.twoDayWS else "0"
                each_center_21_day.text = if (!item.twOneDay.isNullOrBlank()) item.twOneDay else "0"
                each_center_nwet.text = if (!item.nwet.isNullOrBlank()) item.nwet else "0"
                each_center_mmbk.text = if (!item.mmbk.isNullOrBlank()) item.mmbk else "0"
